...
public class Constant {
	 ...
	 */
	public static final String PROJECT = "Zoho";
	...
	/**
	 * This constant holds the EMAIL address of testteam.developer@gmail.com, which
	 * will be filled while creating a new user
	 */
	...
	// public static final String TEST_TEAM_DEVELOEPR_EMAIL1 =
	// "s.shwetha@cssindiaonline.com";

	/**
	 * To Register a new User in Commercial Website We need to have a unique email
	 * 
	 * @author Shailendra 16-Apr-2020
	 */
	public static String getUnique_DeveloperEmail() {
		String emailParts[] = TEST_TEAM_DEVELOPER_YDT_EMAIL.split("@");
	...
	/**
	 * controls the driver object parameters like : explicit wait time implicit wait
	 ...
	 */
	private static final String serverExesFolderName = "DriverBinaries";
	...
	public static final String chromeDriverFileName_Linux = "chromedriver";
	...
	private static final String testDataFolderName = "Resources";
	...
	public static final String resultFolderName = "Report";
	...
	private static String environmentInfoSheet = "LIVE";
	...
	public static void setEnvironmentInfoSheet(String environment) {
		if (environment != null && !environment.equals("") && !environment.equals("${environment}")) {
			...
			System.err.println("Warning: TestNG parameter 'environment' value= {" + environment
					+ "} is incorrectly provided, running the tests on '" + environmentInfoSheet + "' enviroment");
		}
	}
	...
	public static void setTestDataFilePath(String absPath) {
		if(null != absPath && !absPath.equals("") && !absPath.contains("${sheetAbsPath}")) {
			testDataFilePath = absPath;
		}
	}
	...
	/**
	 * This method checks, whether the Downloads_Qlik folder exists in [C:/]
	 * directory in windows, and [/var/lib/jenkins/workspace/] directory in
	 * linux/jenkins If it is not found then a new folder will be created
	 * 
	 * @return absolute path of Downloads_Qlik folder as per
	 *         OS(C:\Downloads_Qlik\IOTRON)
	 */
	public static String getQlikDownloadsPath() {
		...
		if (!f.exists()) {
			if (f.mkdirs()) {
				Reporter.report(STATUS.INFO, "DIRECTORY CREATED: " + folderDir);
			} else {
				Reporter.report(STATUS.ERROR, "DIRECTORY CREATION FAILED: " + folderDir);
	...
	public static String getFirefoxDriverLocation(String platform) {
		...
		if (platform.toLowerCase().contains("lin")) {
			...
			val = root + "/" + serverExesFolderName + "/selenium_standalone_binaries/windows/marionette/" + bit + "/"
					+ geckoDriverFileName_Win;
		}
	...
	public static String getChromeDriverLocation() {
		...
		String val = "";
		if (platform.toLowerCase().contains("lin")) {
			val = root + "/" + serverExesFolderName + "/selenium_standalone_binaries/linux/googlechrome/" + bit + "/"
					+ chromeDriverFileName_Linux;
		} else if (platform.toLowerCase().contains("win")) {
			val = root + "/" + serverExesFolderName + "/selenium_standalone_binaries/windows/googlechrome/" + bit + "/"
					+ chromeDriverFileName_Win;
		}
		System.out.println("SHAILENDRA: " + val);
		return val;
	}
	...
	public static String getIEDriverLocation() {
		String val = root + "/" + serverExesFolderName + "/selenium_standalone_binaries/windows/internetexplorer/" + bit
				+ "/" + ieDriverFileName;
		return val;
	}

	public static String getEdgeDriverLocation() {
		String val = root + "/" + serverExesFolderName + "/selenium_standalone_binaries/windows/edge/" + bit + "/"
				+ edgeDriverFileName;
		return val;
	}
