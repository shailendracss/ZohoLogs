...
public class Util {
	...
	public static void killExcelProcess() {
		try {
			...
			if (platform.toLowerCase().contains("win")) {
				Reporter.report(STATUS.INFO,"Killing Excel Process on " + platform);
			...
			if (dir.isDirectory()) {
				...
				do {
					...
					if (timeOutTimeCounter > 0) {
						...
						timeOut = false;
						Reporter.report(STATUS.INFO, "TIMEOUT FILE [" + fileNameContains
	...
	public static String moveFileToDirectory(String filePath, String destinationFolder) {
		...
		try {
			...
			if (fSrc.exists()) {
				...
				Reporter.report(STATUS.INFO, "File [" + fileName + "] moved to " + destinationFolder);
			}
		} catch (IOException e) {
			new CustomExceptionHandler(e);
		}
	...
	public static void verifyFile_Downloaded(String fileName, String desc) {
		...
		if (path != null) {
			Reporter.report(STATUS.PASS, desc + " Downloaded " + path);
		} else {
			Reporter.report(STATUS.FAIL, desc + " Download failed");
		}
	}
	...
	public static synchronized boolean forceDelete(String absoluteFilePath) {
		...
		try {
			...
			if(file.exists()){
				...
				Reporter.report(STATUS.PASS, "Directory/File Deleted: " + absoluteFilePath);
			}else{
				Reporter.report(STATUS.INFO, "Directory/File Not Exist: " + absoluteFilePath);
			}
	...
	public static void comparator_PageValues(String expected, String onPage, String objectAndpageDesc) {
		...
		if (act.contains(exp)) {
			Reporter.report(STATUS.PASS, "Displayed Value of [<b>" + objectAndpageDesc + "</b>] : '<b>" + onPage
					+ "</b>' has matched with expected value: '<b>" + expected + "</b>'");
		} else {
			Reporter.report(STATUS.FAIL, "Displayed Value of [<b>" + objectAndpageDesc + "</b>] : '<b>" + onPage
	...
	public static boolean comparator_NonNumbers(String expected, String onPage, String desc) {
		if (expected.toLowerCase().contains(onPage.toLowerCase())
			...
			Reporter.report(STATUS.PASS, "Expected value of [<b>" + desc + "</b>] : '<b>" + expected
					+ "</b>' and on Page value displayed is '<b>" + onPage + "</b>'");
			return true;
		} else {
			Reporter.report(STATUS.FAIL, "Expected value of [<b>" + desc + "</b>] : '<b>" + expected
					+ "</b>' and on Page value displayed is '<b>" + onPage + "</b>'");
			return false;
		}
	}
	...
	public static void comparator(String calculated, String onPage, String desc) {
		...
		if (compareNumeric(Util.removeCommas(calculated), Util.removeCommas(onPage))) {
			Reporter.report(STATUS.PASS, "Calculated value of [<b>" + desc + "</b>] : '<b>" + calculated
					+ "</b>' and on Page value displayed is '<b>" + onPage + "</b>'");
		} else {
			Reporter.report(STATUS.FAIL, "Calculated value of [<b>" + desc + "</b>] : '<b>" + calculated
					+ "</b>' and on Page value displayed is '<b>" + onPage + "</b>'");
		}
	...
	public static void comparator_List_NonNumbers(List<String> expectedStrings, List<String> onPage, String desc) {
		...
		if (flag1 && flag2) {
			Reporter.report(STATUS.PASS, "All values of [<b>" + desc + "</b>] matched with : " + expectedStrings);
		}
	...
	public static void comparator_List(List<String> calculatedOrExpected, List<String> onPage, String desc) {
		try {
			if (onPage.containsAll(calculatedOrExpected)) {
				Reporter.report(STATUS.PASS, "Values of [<b>" + desc + "</b>] are properly displayed as <b>"
	...
	public static String renameDownloadedFile(String fileNameContains, String newFileNameWithExtension) {
		...
		if (filePathWithOldName != null) {
			...
			if (fOld.renameTo(fNew)) {
				Reporter.report(STATUS.INFO, "Downloaded file successfully renamed to ["
						+ newFileNameNoSpecialChar + "] stored on path [" + filePathWithNewName + "]");
			} else {
				Reporter.report(STATUS.INFO, "Downloaded file renaming failed, Downloaded File : ["
