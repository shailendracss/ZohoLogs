	...
	public static synchronized void createHTML_NonXl(){
		...
		try{
			...
			reportIn = "<html>" + "<head>"
				...
				+ "<span class='properties-span'>Parallel Mode:<b> "+ Scenario.getInParallel() +"</b></span>"
				...
				+ "<td colspan='8' text-align='center' class='dashboard-header' width='50px'>"
					...
					+ "Suite:" + Scenario.getTestNG_SuiteName() + ", "
					+ "Test: </span>"
				+" </td></tr>"
				...
				+ "<tr>"
				+ "<td colspan='3' style='text-align: center;'>"+ new SimpleDateFormat("dd/MM/yyyy hh:mm:ss a").format(Scenario.getTestExecutionStartDate())+ "</td>"
				+ "<td colspan='3' style='text-align: center;'>"+ new SimpleDateFormat("dd/MM/yyyy hh:mm:ss a").format(Scenario.getTestExecutionEndDate())+ "</td>"
				+ "<td colspan='2' style='text-align: center;'>"+ Scenario.getTestExecutionTime()+ "</td>"
				+ "</tr>"
			...
			for (List<Scenario> tempObj : listOfList) {
				...
				for (int i = 0; i < arraySize; i++) {
					...
					List<Scenario> subStepsList=tempObj.get(i).getList();
