...
public class CustomListener implements ITestListener,IExecutionListener{
	...
	public void onStart(ITestContext context) {
		...
		DriverFactory.startChromeDriverService(testDataMap);
	...
	public void onTestStart(ITestResult result) {
		...
		if(!parallelFlag.equals("none") && !parallelFlag.equals("tests")){
			...
			new Login().load_App_URL().performLogin(testDataMap.get(TestNGKeys.user));
	...
	public void onTestSkipped(ITestResult result) {
		...
		Reporter.report(STATUS.SKIP,"It depends on methods which got failed: '"+(result.getMethod().getMethodsDependedUpon().length>0?Arrays.toString(result.getMethod().getMethodsDependedUpon()):null)+"'");
		Reporter.onTestEnd_NonXl(null);
	...
	public void onTestFailure(ITestResult result) {
		...
		Reporter.report(STATUS.ERROR, "- PREMATURE EXECUTION STOPPED : Failure Exception :- <br/><b style='color:red;font-size: small;font-family: arial,sans-serif;'>"+readableStackTrace+"</b>");
	...
	public void onFinish(ITestContext context) {
		...
		DriverFactory.stopChromeDriverService();
	}
	...
	public void onExecutionFinish() {
		...
		MailManager.sendNotificationMail("Suite["+suiteName+"] Test["+testName+"] Env["+Constant.getEnvironmentInfoSheet()+"] "+new Date());
	}
