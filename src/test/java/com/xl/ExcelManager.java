...
public class ExcelManager {
	...
	public static void createExcelTemplate(String filePath,String sheetName,String... colHeaderNames) {
		ExcelWriter.createExcelTemplate(filePath, sheetName, colHeaderNames);
	}

	public static void killChromeProcess() {
		try{
			Runtime.getRuntime().exec("cmd /c taskkill /f /im "+com.util.Constant.chromeDriverFileName_Win);
	 ...
	 * <pre>
	 * */
	public ExcelManager(String workbookPath,int sheetIndex) {
	 ...
	 * <pre>
	 * */
	public ExcelManager(String workbookPath,String sheetName) {
		this.workbookPath=workbookPath;
		initWorkbookObject();
	...
	public int getRowCount() {
		return ExcelWriter.getRowCount(this);
	}
	...
	public int getLastRowIndex() {
		return ExcelWriter.getLastRowNum(this);
	}
	...
	public void setValue(int rowIndex,int colIndex,String data){
		ExcelWriter.setValue(this,rowIndex, colIndex, data);
	}	
	...
	public void setValue(int rowIndex,String colName,String data){
		ExcelWriter.setValue(this,rowIndex, colName, data);
	}
...
class ExcelWriter{
	...
	public static synchronized int getLastRowNum(ExcelManager tab) {
		init(tab);
		return sheet.getLastRowNum();
	}

	public static synchronized int getRowCount(ExcelManager tab) {
		init(tab);
		return sheet.getLastRowNum()+1;
		// return sheet.getLastRowNum();
	}

	private static synchronized void init(ExcelManager tab) {
		workbookPath=tab.getWorkbookPath();
		workbook=tab.getWorkbook();
		sheet=tab.getSheet();
	}	

	public static synchronized void setValue(ExcelManager tab, int rowNum,int colNum,String data){
	...
	public static synchronized void setValue(ExcelManager tab,int rowNum,String colName,String data){
		Util.killExcelProcess();
		init(tab);
