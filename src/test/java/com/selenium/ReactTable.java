...
public class ReactTable {
	...
	private List<WebElement> getRowsList() {
		if(rowList == null) {
			...
			try{ 
				...
				Reporter.report(STATUS.FAIL, "getRowsList() method failed for locator "+locator+" <br/><br/> Outer HTML "+getWebElement().getAttribute("outerHTML"));
				new CustomExceptionHandler(e);
			}
		}
		return rowList;
	...
	public ReactTable initHeaderColumnList(int headerRow) {
		try{
			...
			Reporter.report(STATUS.WARNING, "initHeaderColumnList() method failed for headerRow "+headerRow+" in table "+locator+" <br/><br/> Exception message "+e.getMessage());
			//new CustomExceptionHandler(e);
		}
		return this;
	}
	...
	public int getColumnNumber(String colName) {
		...
		if (foundColNum==-1) {
			Reporter.report(STATUS.INFO, "Passed Column ["+colName+"] is not found in column header list "+headerColumnList);
		}
	...
	public void verifyColumnHeaders(int headerRow, List<String> expectedColNames) {
		...
		for (int i = 0; i < expectedColNames.size(); i++) {
			...
			if (!tempColName.toLowerCase().contains(expectedColNames.get(i).toLowerCase())) {
				Reporter.report(STATUS.FAIL, "On Row["+headerRow+"] and Col["+(i+1)+"] Column text '" + expectedColNames.get(i) + "' not found but column '"
		...
		if (flag) {
			Reporter.report(STATUS.PASS, "All columns found : " + expectedColNames);
		} else {
			Reporter.report(STATUS.INFO, "COLUMNS EXPECTED: [" + getStringifiedColNames(expectedColNames) + "]");
			Reporter.report(STATUS.INFO, "COLUMNS ACTUAL : [" + getStringifiedColNames(actualColNames) + "]");
		}
	}
