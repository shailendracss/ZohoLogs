...
class SelectCustom extends WebUtils{
		...
		try {
			...
			if(sel.isMultiple()){
				if (elem.getAttribute("innerHTML").contains("<option")) {
					...
					Reporter.report(STATUS.FAIL, "Dropdown does not have any options, Locator: "+getByObjectFromWebElement(elem));
				}
			}else{
				Reporter.report(STATUS.FAIL, "Passed dropdown is not a MultiSelect");
			}
		} catch (Exception e) {
	...
	public void selectByValue(Object element, String value) {
		...
		try {
			...
			if (elem.getAttribute("innerHTML").contains("<option")) {
				...
				Reporter.report(STATUS.FAIL, "Dropdown does not have options to select, Locator : "+getByObjectFromWebElement(elem));
			}
		} catch (Exception e) {
	...
	public void selectByPartialVisibleText(Object element, String text, boolean runReport) {
		...
		try {
			...
			if (runReport) {
				if (flag) {
					Reporter.report(STATUS.PASS, "'" + text + "' successfully selected in shuttle/dropdown old values '"+ oldVal +"'");
				} else {
					Reporter.report(STATUS.FAIL, "'" + text + "' is NOT selected from dropdown");
				}
			}
		}
	}
	...
	public void selectByVisibleText(Object element, String text, boolean runReport) {
		...
		try {
			...
			if (runReport) {
				if (flag) {
					Reporter.report(STATUS.PASS, "'" + text + "' successfully selected in dropdown replacing '"+ oldVal +"'");
				} else {
					Reporter.report(STATUS.FAIL, "'" + text + "' is NOT selected from dropdown");
				}
			}
	...
	public WebElement getFirstSelectedOption(Object element) {
		...
		try {
			...
			if (elem.getAttribute("innerHTML").contains("<option")) {
				...
				Reporter.report(STATUS.FAIL, "Dropdown does not have options, Locator: "+getByObjectFromWebElement(elem));
			}
		} catch (Exception e) {
