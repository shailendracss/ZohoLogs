...
class WebUtils {
	...
	public WebElement getDynamicElement(Object parentElem, By childByObj, String desc, boolean trueForSnapshot) {
		...
		try {
			...
			Reporter.report(STATUS.ERROR, "Element with locator ["+childByObj+"] is Not found");
			new CustomExceptionHandler(e);
		} finally {
			if (!desc.equals("")) {
				if (reportFlag) {
	...
	public WebElement getDynamicElement(By byObj, String desc, boolean trueForSnapshot) {
		...
		try {
			...
			Reporter.report(STATUS.ERROR, "Element with locator ["+byObj+"] is Not found");
			new CustomExceptionHandler(e);
		} finally {
			if (!desc.equals("")) {
				if (reportFlag) {
		...
		try {
			...
			if (!desc.equals("")) {
				if (bool) {
					...
					Reporter.report(STATUS.FAIL, desc +" is NOT clicked");
				}
	 ...
	 * Bring the Element into Screen MIDDLE view, and also changes its background to
	 * Yellow
	 * 
	 * @param element
	 *            A WebElement/By object
	...
	public void javaScript_ScrollIntoMIDDLEView_AndHighlight(Object element) {
		try {
			...
			//new CustomExceptionHandler(e);
			Reporter.report(STATUS.WARNING, "javaScript_ScrollIntoMIDDLEView_AndHighlight failed to execute on "+getByObjectFromWebElement(element));
		}
	}
	...
	public void javaScript_ScrollIntoBOTTOMView_AndHighlight_ThenClick(Object element,String desc) {
		...
		try {
			...
			if (desc!=null && !desc.equals("")) {
				Reporter.report(STATUS.INFO, "Clicking on "+desc);
			}
